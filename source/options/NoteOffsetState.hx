...
class NoteOffsetState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		video = new VideoSprite(Paths.video('tras${FlxG.random.int(0, 2)}'), true, false, true, true);
		video.play();
		insert(0, video);
		...
		var note:FlxText = new FlxText(12, FlxG.height - 24, 0, "Note: Press Z to hide/show boyfriend and girlfriend.", 12);
		note.cameras = [camHUD];
		note.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(note);
	...
	override public function update(elapsed:Float)
	{
		...
		if(FlxG.keys.justPressed.Z) {
			for (i in [boyfriend, gf]) i.visible = !i.visible;
			trace("hola");
		}
